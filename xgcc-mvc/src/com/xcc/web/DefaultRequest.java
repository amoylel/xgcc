 ...
 * boundary=----WebKitFormBoundary0YXRJFPyA1aqN0t1,isMultipartContent=true (需要注意的是: xcc框架在使用的过程中,如果配置文件上传的临时目录找不到时,
 * 这种方式无法获取数据)</li>
...
public class DefaultRequest implements HttpServletRequest {
	...
	@Override
	public FilePart getPart(String arg0) throws IOException, ServletException {
	...
	public Collection<Part> getParts() throws IOException, ServletException {
		...
		for (Part part : collection) {
			if(part != null && part.getSize() > 0) {
				partResult.add(new FilePart(part));
			}
		}
		return partResult;
	}

	public FilePart[] getParts(String arg0) throws IOException, ServletException {
		if(this.isMultipartContent == false) {
		...
		List<FilePart> partResult = new ArrayList<FilePart>();
		for (Part part : collection) {
			if(part != null && part.getSize() > 0 && StringUtils.equals(arg0, part.getName())) {
				partResult.add(new FilePart(part));
			}
		}
		return partResult.toArray(new FilePart[partResult.size()]);
	}
